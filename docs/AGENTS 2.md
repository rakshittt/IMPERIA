...
IMPERIA integrates an adapted methodology pack inspired by Anthropic's
Apache-2.0 financial-services examples. The pack lives in
`tradingagents/expert_agents/skill_pack.py` and is injected into each DeepSeek
agent prompt. It is also attached to every agent output as `methodology`.

The pack improves agent discipline in areas such as event materiality, earnings
variance analysis, valuation comparability, competitive-landscape framing,
ownership-signal caveats, disconfirming evidence, research-factor generation,
and citation/source-quality auditing.

This is not a second runtime and it does not add Anthropic model calls. IMPERIA
continues to use DeepSeek for agent reasoning and keeps all outputs educational,
source-cited, and non-prescriptive.
