...
class NormalizedChatOpenAI(ChatOpenAI):
    ...
    Some OpenAI-compatible APIs return content as a list of typed blocks
    (reasoning, text, etc.). ``invoke`` normalizes to string for consistent
    downstream handling.
    """
...
class DeepSeekChatOpenAI(NormalizedChatOpenAI):
    ...
    Two DeepSeek API quirks are handled here:

    1. **Thinking-mode round-trip.** When thinking models return
...
_PASSTHROUGH_KWARGS = (
    "timeout", "max_retries", "api_key", "callbacks", "http_client", "http_async_client",
)

# DeepSeek is exposed through the OpenAI-compatible Chat Completions API.
_PROVIDER_CONFIG = {
...
class OpenAIClient(BaseLLMClient):
    """DeepSeek client implemented through LangChain's OpenAI-compatible chat adapter."""
