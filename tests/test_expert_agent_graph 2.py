...
@pytest.mark.unit
def test_freshness_handles_mixed_naive_and_aware_timestamps():
    freshness = freshness_from_bundle(
        {
            "citations": [
    ...
    assert freshness.oldest_input_ts is not None
    assert freshness.newest_input_ts is not None
    assert freshness.oldest_input_ts.tzinfo is not None


@pytest.mark.unit
def test_runtime_emit_event_handles_empty_warning_list(monkeypatch):
    runtime = ExpertAgentRuntime()
    monkeypatch.setattr(runtime, "assemble_evidence_bundle", lambda ticker, query, intent, window: _bundle() | {"intent": intent, "window": window, "query": query})
    events = []
    result = runtime.run(
        query="Deep research report on AAPL",
        ticker="AAPL",
        mode="deep",
        emit_event=lambda event, **payload: events.append((event, payload)),
    )
    assert result.mode == "deep"
    assert any(event == "agent_completed" for event, _payload in events)
