...
def test_parse_nl_screener_query_skips_deepseek_when_deterministic(monkeypatch):
    monkeypatch.setenv("DEEPSEEK_API_KEY", "key")
    monkeypatch.setattr(screener, "deepseek_text", lambda *args, **kwargs: (_ for _ in ()).throw(AssertionError("DeepSeek should not be called")))
    criteria = screener.parse_nl_screener_query("profitable technology stocks with P/E under 35")
    assert criteria.max_pe == 35
